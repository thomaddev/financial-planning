...
import { removeDuplicates, currencyFormatterForAggrid } from '@vise/kit'
...
import {
  ...
  generateGridCurrent,
  generateGridPlan,
  updateValuesForKeysStartingWithM,
} from '@/components/aggrids/utilities'
import { FIELD_MONTHS, MONTH_FEILD, TEMPLATE_DOCTYPE } from '@/data'
import { useSavePlanning, useGetPlanning, useSubmitPlanning } from '@/lib/api/planning'
...
export default function Employee() {
  ...
  const {
    ...
    gridRef,
    docType: TEMPLATE_DOCTYPE.employee,
    setRowData,
  })

  // Fetch employee data using the reusable hook
  const { data: employeeData } = useGetPlanning(id)

  // Set form header when data changes
  useEffect(() => {
    if (employeeData && id !== 'new') {
      setFormHeader(employeeData)
      setRowData(employeeData.budget_items)
    }
  }, [employeeData, id, setFormHeader, setRowData])
  ...
  const submitForm = useCallback(async () => {
    ...
    try {
      ...
      const budgetPlans = gridData.map((row) => ({
        ...row,
        document_type: TEMPLATE_DOCTYPE.employee,
      }))

      const params = {
        ...formHeader,
        template_type: TEMPLATE_DOCTYPE.employee,
  ...
  const calculateValueSetter = useCallback((params: any) => {
    ...
    if (field === 'template_link_detail.quantity') {
      year1Value = newValue * data.template_link_detail.rate * data.template_link_detail.duration
      params.data.template_link_detail.quantity = newValue
    } else if (field === 'template_link_detail.rate') {
      year1Value =
        data.template_link_detail.quantity * newValue * data.template_link_detail.duration
      params.data.template_link_detail.rate = newValue
    } else if (field === 'template_link_detail.duration') {
      year1Value = data.template_link_detail.quantity * data.template_link_detail.rate * newValue
      params.data.template_link_detail.duration = newValue
    }

    const updateItem = {
      ...params.data,
      ...updateValuesForKeysStartingWithM(params.data),
      year_1: year1Value,
      year_2: year1Value,
      year_3: year1Value,
      year_4: year1Value,
    ...
    const rowNode = params?.api?.getRowNode(updateItem.gridId)
    rowNode?.updateData(updateItem)
    params.data.october = year1Value
    return true
  }, [])
