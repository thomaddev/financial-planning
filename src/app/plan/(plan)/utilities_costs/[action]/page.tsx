...
export default function UtilitiesCosts() {
  ...
  const {
    ...
    gridRef,
    docType: TEMPLATE_DOCTYPE.utilities_costs,
    setRowData,
  })

  // Fetch utilities costs data using the reusable hook
  const { data: utilitiesCostsData } = useGetPlanning(id)

  // Set form header when data changes
  useEffect(() => {
    if (utilitiesCostsData && id !== 'new') {
      setFormHeader(utilitiesCostsData)
      setRowData(utilitiesCostsData.budget_items || [])
    }
  }, [utilitiesCostsData, id, setFormHeader, setRowData])
  ...
  const submitForm = useCallback(async () => {
    ...
    try {
      ...
      const budgetPlans = gridData.map((row) => ({
        ...row,
        document_type: TEMPLATE_DOCTYPE.utilities_costs,
      }))

      const params = {
        ...formHeader,
        template_type: TEMPLATE_DOCTYPE.utilities_costs,
