...
export default function MaterialCost() {
  ...
  const {
    ...
    gridRef,
    docType: TEMPLATE_DOCTYPE.material_cost,
    setRowData,
  })

  // Fetch material cost data using the reusable hook
  const { data: materialCostData } = useGetPlanning(id)

  // Set form header when data changes
  useEffect(() => {
    if (materialCostData && id !== 'new') {
      setFormHeader(materialCostData)
      setRowData(materialCostData.budget_items)
    }
  }, [materialCostData, id, setFormHeader, setRowData])
  ...
  const submitForm = useCallback(async () => {
    ...
    try {
      ...
      const budgetPlans = gridData.map((row) => ({
        ...row,
        document_type: TEMPLATE_DOCTYPE.material_cost,
      }))

      const params = {
        ...formHeader,
        template_type: TEMPLATE_DOCTYPE.material_cost,
