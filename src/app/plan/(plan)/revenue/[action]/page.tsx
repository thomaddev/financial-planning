...
export default function Revenue() {
  ...
  const {
    ...
    gridRef,
    docType: TEMPLATE_DOCTYPE.revenue,
    setRowData,
  })

  // Fetch revenue data using the reusable hook
  const { data: revenueData } = useGetPlanning(id)

  // Set form header when data changes
  useEffect(() => {
    if (revenueData && id !== 'new') {
      setFormHeader(revenueData)
      setRowData(revenueData.budget_items)
    }
  }, [revenueData, id, setFormHeader, setRowData])
  ...
  const submitForm = useCallback(async () => {
    ...
    try {
      ...
      const budgetPlans = gridData.map((row) => ({
        ...row,
        document_type: TEMPLATE_DOCTYPE.revenue,
      }))

      const params = {
        ...formHeader,
        template_type: TEMPLATE_DOCTYPE.revenue,
  ...
  const calculateValueSetter = useCallback((params: any) => {
    ...
    if (field === 'template_link_detail.quantity') {
      ...
      year1Value = data.template_link_detail.quantity * newValue
