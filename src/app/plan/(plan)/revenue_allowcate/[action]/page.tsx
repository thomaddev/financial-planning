...
export default function RevenueAllocate() {
  ...
  const {
    ...
    gridRef,
    docType: TEMPLATE_DOCTYPE.revenue_allowcate,
    setRowData,
  })

  // Fetch revenue allocate data using the reusable hook
  const { data: revenueAllocateData } = useGetPlanning(id)

  // Set form header when data changes
  useEffect(() => {
    if (revenueAllocateData && id !== 'new') {
      setFormHeader(revenueAllocateData)
      setRowData(revenueAllocateData.budget_items || [])
    }
  }, [revenueAllocateData, id, setFormHeader, setRowData])
  ...
  const submitForm = useCallback(async () => {
    ...
    try {
      ...
      const budgetPlans = gridData.map((row) => ({
        ...row,
        document_type: TEMPLATE_DOCTYPE.revenue_allowcate,
      }))

      const params = {
        ...formHeader,
        template_type: TEMPLATE_DOCTYPE.revenue_allowcate,
