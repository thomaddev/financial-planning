...
export default function LandAndConstruction() {
  ...
  const {
    ...
    gridRef,
    docType: TEMPLATE_DOCTYPE.land_and_construction,
    setRowData,
  })

  // Fetch land and construction data using the reusable hook
  const { data: landAndConstructionData } = useGetPlanning(id)

  // Set form header when data changes
  useEffect(() => {
    if (landAndConstructionData && id !== 'new') {
      setFormHeader(landAndConstructionData)
      setRowData(landAndConstructionData.budget_items || [])
    }
  }, [landAndConstructionData, id, setFormHeader, setRowData])
  ...
  const submitForm = useCallback(async () => {
    ...
    try {
      ...
      const budgetPlans = gridData.map((row) => ({
        ...row,
        document_type: TEMPLATE_DOCTYPE.land_and_construction,
      }))

      const params = {
        ...formHeader,
        template_type: TEMPLATE_DOCTYPE.land_and_construction,
