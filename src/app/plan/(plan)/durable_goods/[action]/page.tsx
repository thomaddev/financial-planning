...
export default function DurableGoods() {
  ...
  const {
    ...
    gridRef,
    docType: TEMPLATE_DOCTYPE.durable_goods,
    setRowData,
  })
  // Fetch durable goods data using the reusable hook
  const { data: durableGoodsData } = useGetPlanning(id)

  // Set form header when data changes
  useEffect(() => {
    if (durableGoodsData && id !== 'new') {
      setFormHeader(durableGoodsData)
      setRowData(durableGoodsData.budget_items || [])
    }
  }, [durableGoodsData, id, setFormHeader, setRowData])
  ...
  const submitForm = useCallback(async () => {
    ...
    try {
      ...
      const budgetPlans = gridData.map((row) => ({
        ...row,
        document_type: TEMPLATE_DOCTYPE.durable_goods,
      }))

      const params = {
        ...formHeader,
        template_type: TEMPLATE_DOCTYPE.durable_goods,
  ...
  const calculateValueSetter = useCallback((params: any) => {
    ...
    if (field === 'template_link_detail.quantity_request') {
      year1Value = newValue * data.template_link_detail.per_price
      params.data.template_link_detail.quantity_request = newValue
    } else if (field === 'template_link_detail.per_price') {
      year1Value = data.template_link_detail.quantity_request * newValue
      params.data.template_link_detail.per_price = newValue
    }
